...
class Login_m extends CI_Model
{
    public function check_login($login, $password)
    {
        ...
        foreach ($data['users'] as $check) {
            if (($login == $check->login || $login == $check->email) && (password_verify($password, $check->password)) && $check->activation == '1') {
                $session_data['id'] = $check->id;
                $session_data['first_name'] = $check->name;
                $session_data['last_name'] = $check->surname;
                $session_data['email'] = $check->email;
                $session_data['name'] = $check->name;
                $session_data['rola'] = $check->privilages;
